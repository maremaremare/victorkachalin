...
(function($){
	$.fn.slider=function(options){
		var defaults={
			interval:6000,   //the interval between changing the images when autoplay is turned on (in miliseconds)
		...
		var api, timer=-1, images=[], current, root, thumbContainer, containerNum=0, inAnimation=false, descBox, descBottom=2;
		...
		function setDescription(){
			...
			descBox=$('<div id="description-box" >bla bla</div>');
		...
		function printScrollable(){
			thumbContainer.append('<div class="items" />');
			for(var i in images){
				if(images.hasOwnProperty(i)){
					if(i%options.imgPerScroll===0){
						thumbContainer.find('.items:first').append('<div/>');
						containerNum++;
					}
					thumbContainer.find('.items:first div:last').append('<img src="'+images[i].attr('src').replace('_big', '')+'" />');
				}
			}
